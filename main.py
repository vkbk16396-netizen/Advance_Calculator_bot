...
def safe_locals(chat_id: int) -> dict:
    ...
    def sin_deg(v):
        return sp.sin(sp.pi * v / 180)

    def cos_deg(v):
        return sp.cos(sp.pi * v / 180)

    def tan_deg(v):
        return sp.tan(sp.pi * v / 180)
    ...
    # ===== HELP (REDESIGNED) =====
        ...
        if img is None:
            await asyncio.to_thread(
                bot.send_message,
                chat_id,
                "❌ Invalid function(s) for plotting"
            )
        else:
        ...
        try:
            ...
            await asyncio.to_thread(
                bot.send_message,
                chat_id,
                f"❌ Solve error: {str(e)[:120]}"
            )

    elif lower == "/export":
        async with db_lock:
            cursor.execute(
                "SELECT expr, result FROM history WHERE chat_id=?",
                (chat_id,)
            )
        ...
        if conversion is not None:
            await asyncio.to_thread(bot.send_message, chat_id, f"🔄 {conversion}")
            return {"ok": True}
